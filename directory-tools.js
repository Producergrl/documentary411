  ...
  window.D411 = {
    renderDirectory(config){
      ...
      function draw(){
        ...
        document.querySelector('#d411count').textContent = `${filtered.length} resource${filtered.length===1?'':'s'} shown. Use Suggest correction if a deadline, fee, or status has changed.`;
        document.querySelector('#d411cards').innerHTML = filtered.length ? filtered.map(card).join('') : '<div class="d411-empty">No resources match those filters yet. Try fewer filters or submit a resource we should add.</div>';
      }
